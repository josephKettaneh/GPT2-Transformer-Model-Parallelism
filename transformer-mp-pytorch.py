...
import logging
import os
logging.basicConfig(level=logging.INFO,filename='train_log_TRANS-3GPUS-1-nodes-V100.log', filemode='a') 

logging.info('Working on {:,} GPUs'.format((torch.cuda.device_count())))

num_workers = int(os.environ['SLURM_NTASKS'])
logging.info('Working on {:,} workers'.format((num_workers)))
...
free_memory = torch.cuda.memory_allocated() / (1024 ** 2)
print("Total GPU memory: {:.2f} MB".format(total_memory))
print("Free GPU memory: {:.2f} MB".format(free_memory))
...
logging.info('data-loading-done with train len = : {:,}'.format(len(train_data)))
...
logging.info('batchify is done with batch size = : {:,}'.format(batch_size))
...
ntokens = vocab_len #len(vocab) # the size of vocabulary
...
worker_name = os.environ['SLURM_JOB_NAME']

NODE_ID = os.environ['SLURM_NODEID']
MASTER_ADDR = os.environ["SLURM_SRUN_COMM_HOST"]
MASTER_PORT = "29500"#os.environ["MASTER_PORT"]
...
rpc.init_rpc("worker0", rank=rank, world_size=world_size, rpc_backend_options=rpc.TensorPipeRpcBackendOptions(
        init_method="file://{}".format(tmpfile.name),
        _transports=["uv"],
        _channels=["cuda_ipc", "cuda_basic"],
))
...
logging.info('rpc.init_rpc is done with batch size = : {:,}'.format(batch_size))
...
logging.info('Total parameters in model: {:,}'.format(get_total_params(model)))
...
import time
def train():
    logging.info('Training now  of model with params: {:,}'.format(get_total_params(model)))
    model.train() # Turn on the train mode
    total_loss = 0.
    start_time = time.time()
    ntokens = vocab_len #len(vocab)
...
print('=' * 89)
print('| End of training | Total time: {:5.2f}s | | test loss {:5.2f} | test ppl {:8.2f}'.format(
    (train_end_time - train_start_time),  test_loss, math.exp(test_loss)))
print('=' * 89)